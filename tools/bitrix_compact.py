...
KEEP = ["IE_CODE", "IE_NAME", "IE_ID", "IE_ACTIVE",
        ...
        # Винтовой блок: 22670 «Винтовой блок» + 23168 (свойство с тем же названием).
        # Добавлены 25.08 по замечанию заказчика: в шаблоне сравнения это колонка ключа,
        # а в компакте её не было вовсе — все 2 701 винтовая карточка Enger уезжала в
        # умолчание «Baosi». До пересборки компакта блок выводится из кода модели
        # (`enger_specs.enger_block`), но чинить надо здесь: правило по префиксу знает
        # только серии HC/HB/HA/BS, а это 1 684 карточки из 2 701.
        "IP_PROP22670", "IP_PROP23168"]

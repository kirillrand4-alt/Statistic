...
def main_price(page: str) -> str:
    ...
    # Микроразметка идёт ПЕРВОЙ, когда она единственная и однозначная. Расстояние до h1
    # ошибается там, где рядом с заголовком стоит не цена: у magnus-prom ближе всего
    # блок «экономия: 150 055 ₽», и в выгрузку уезжала сумма скидки вместо цены
    # (84 карточки из 205, медиана — 19% от настоящей цены; AE1-45/16 А LD: 150 055
    # вместо 787 970 при зачёркнутой 938 025). itemprop="price" на таких карточках один
    # и указывает ровно на текущую цену.
    tagged = {m.group(1) for m in re.finditer(r'itemprop="price"[^>]*content="(\d{3,9})"',
                                              html.unescape(page))}
    if len(tagged) == 1:
        v = tagged.pop()
        if 1000 <= int(v) <= 100_000_000:
            return v
    h1 = re.search(r"<h1[^>]*>(.*?)</h1>", page, re.S)
    if not h1:
        return ""

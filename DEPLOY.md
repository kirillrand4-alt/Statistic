...
окнами по 10 дней **от новых дат к старым** (от `--to`, по умолчанию вчера),
готовность проверяется раз в 3 минуты (создаёт → ждёт → качает → чистит у
Яндекса; окно, не готовое за 40 мин, отменяется и пропускается — доберётся
позже). 429/сбои API не роняют закачку — запрос повторяется. Нужен тот же
токен Яндекса со scope `metrika:read`.
```bash

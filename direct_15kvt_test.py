...
OUT="/home/user/Statistic/Direct_15kvt_TEST.csv"
...
for r in csv.DictReader(open(SRC,encoding="utf-8-sig"),delimiter=";"):
    ...
    prods.append(dict(brand=b, core=core, bar=bar, url=(r.get("URL") or "").strip()))
...
for p in prods:
    ...
    h1=trim(f"Купить компрессор {p['core']} по спец цене",56)
    h2="Компрессор Центр"
    txt=trim(f"Надежный поставщик компрессоров {p['brand']} — нам доверяют лидеры рынка. Звоните!",81)
    disp_link=trim(re.sub(r"\s+","-",broad),50)
    gmin=group_minus(p)
    ...
    for i,(phrase,bid) in enumerate(phrases):
        ...
        row[40]=p["url"]; row[41]=disp_link; row[46]=bid
        if i==0: row[60]=gmin                       # минус-фразы на группу — в первой строке группы
        rows.append(row)

with open(OUT,"w",encoding="utf-8-sig",newline="") as fh:
    w=csv.writer(fh,delimiter=";")
    w.writerow(["Предложение текстовых блоков для кампании"]+[""]*(NCOL-1))

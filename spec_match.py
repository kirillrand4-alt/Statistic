...
# ОТРИЦАТЕЛЬНЫЙ РЕЗУЛЬТАТ 19.08 (проверено, откачено): дописать в _DESCR категорийную прозу
# осушителей («типа», «рефрижераторный», «воздушный», «dryer»…). _DESCR общий, и слово
# «dryer» тут же перестало отличать базовый Ingersoll Rand UP5 11-10-500 от его исполнения
# «UP5 11-10-500 Dryer» — со встроенным осушителем и другой ценой: 55 наших карточек стали
# неоднозначными, +110 лишних пар. Категорийные слова живут в _CAT_WORDS (brand_spec_review)
# и снимаются ТОЛЬКО вне компрессоров, где ключ и так строится заново.
...
def model_code(name, brand):
    ...
    for t in re.split(r"[^0-9a-zа-яё.+]+", s):
        ...
        # _HOMOGLYPH (визуальная свёртка) — ТОЛЬКО для сверки с именем бренда: наш каталог
        # пишет «Осушитель рефрижераторный "АВАС" DRY 1040» кириллицей, и фонетический
        # _CYR2LAT даёт «avaс» (с — кириллическая, без пары), то есть бренд не узнаётся и
        # уезжает в семейство — 17 карточек ABAC. Расширять свёртку на весь токен нельзя:
        # кириллическое «РН» у ЗИФ визуально даёт «ph», а это стоп-слово (см. _visual).
        if not t or (len(t) > 1 and t in _DESCR) or t in btoks \
           or t.translate(_CYR2LAT) in btoks or t.translate(_HOMOGLYPH) in btoks: continue
...
def match(o, cands):
    ...
    ОТРИЦАТЕЛЬНЫЙ РЕЗУЛЬТАТ 19.08 (проверено, не добавляем): пускать расхождение давления
    больше 4%, если сошлись производительность, масса И все три габарита. Идея была снять
    округление (наш ARIACOM NT+132 «8 бар» против их «7,5» при 23 000 л/мин, 2800 кг и
    1430x2020x2800 до миллиметра). Проверка агентом 14 новых пар: 9 ЛОЖНЫХ. Защита по
    производительности пробивается двумя способами — у ЗИФ она зашита в само имя модели
    (ПВ-16 = 16 м³/мин при любом давлении, совпадает всегда), а у Comaro и BERG соседние
    давления различаются на 2,6-2,9%, то есть внутри допуска. Масса и габариты тоже не
    спасают: у ЗИФ на всю линейку одно шасси. В 4 случаях у того же продавца лежала готовая
    карточка с нашим давлением, ещё в 5 — правильная карточка была у нас, и правило уводило
    матч крест-накрест на соседний SKU.

    Послабление по массе (weight_confirms) — ФОЛБЭК В ПРЕДЕЛАХ ПЛОЩАДКИ: если у этого
    продавца строгий проход уже нашёл кандидата, послабление к его карточкам не
    применяется. Иначе оно уводит пару к соседу при живой точной карточке: наш Magnus
    АА1-250A-M-F LD 10 сцепился с их АА1-220A (вес у обоих 4300 кг), хотя рядом у того
    же продавца лежит АА1-250A с ценой копейка в копейку нашей (доказанная ложная пара
    19.08). Защита по «семейному» весу там не срабатывает — ключ серии у Magnus
    вырожден: все 565 карточек АА1-* сидят в одном ('aa', 1).

    Именно по площадке, а не по всему списку: продавцы ведут каталог независимо, и
    точная карточка у одного не значит, что у другого она есть. Замер: границей «весь
    список» терялись 23 доказанно верные пары (ABAC MICRON 2.2, Atlas G7 10FF), потому
    что строгий проход находил их у ДРУГОГО продавца и фолбэк не включался."""
    strict = _match_pass(o, cands, False)
    ok = {c["site"] for c in strict}
    return strict + [c for c in _match_pass(o, cands, True)
                     if c["site"] not in ok and c not in strict]


def _match_pass(o, cands, by_weight):
    out = []
    for c in cands:
        if o["sn"] != c["sn"]: continue
        if not agree_num(o["kw"], c["kw"]) and not (by_weight and weight_confirms(o, c)): continue

...
def svodnaya_etalon(wb, rows, real):
    ...
    for fam in sorted(groups):
        ...
        # Единственная поправка: в диапазон включена НАША цена (H). У заказчика шкала
        # считалась только по конкурентам, и на полном каталоге разваливалась — из 663
        # строк с аналогами в 225 аналог всего один: он одновременно min и max, поэтому
        # красился зелёным всегда. С нашей ценой в диапазоне сравнивать есть с чем при
        # любом числе аналогов, и градиент отсчитывается от неё: конкурент дешевле нас
        # уходит в красный, дороже — в зелёный.
        ws.conditional_formatting.add(
            f"H{top} {get_column_letter(b0)}{top}:{get_column_letter(b0+len(BRANDS)-1)}{top}",
...
def save(rows, cands, out=OUT):
    ...
    lines = [
        ...
        ("  цены брендов", "трёхцветная шкала образца, построчно: самая низкая цена в "
                           "строке красная, средняя жёлтая, самая высокая зелёная. В шкалу "
                           "включена НАША цена — поэтому градиент отсчитывается от неё, и "
                           "конкурент дешевле нас уходит в красный, дороже в зелёный. У "
                           "заказчика шкала считалась только по конкурентам и на полном "
                           "каталоге разваливалась: в 225 строках аналог всего один, он "
                           "одновременно min и max и красился зелёным всегда."),

...
def svodnaya_etalon(wb, rows, real):
    ...
    for fam in sorted(groups):
        ...
        # Трёхцветная шкала образца по короткому блоку брендов — построчно и с его же
        # цветами (min FFF8696B красный, середина FFFFEB84, max FF63BE7B зелёный).
        # Поправка против образца — точки привязки. У заказчика они min/percentile/max,
        # то есть шкала растягивается по ценам конкурентов в строке и от нашей цены никак
        # не зависит: при одном аналоге он одновременно min и max и красится зелёным
        # всегда (таких строк 225 из 663). Здесь пороги заданы формулами от НАШЕЙ цены:
        # ровно наша цена — жёлтый, вдвое-втрое дешевле — красный, заметно дороже —
        # зелёный. Коэффициенты 0,6 и 1,6 сняты с данных: отношение «их цена / наша» по
        # 2 061 паре имеет 5-й квантиль 0,55, медиану 0,97 и 90-й 1,58, так что шкала
        # накрывает реальный разброс, а не теоретический.
        ws.conditional_formatting.add(
            f"{get_column_letter(b0)}{top}:{get_column_letter(b0+len(BRANDS)-1)}{top}",
            ColorScaleRule(start_type="formula", start_value=f"$H{top}*0.6",
                           start_color="FFF8696B",
                           mid_type="formula", mid_value=f"$H{top}", mid_color="FFFFEB84",
                           end_type="formula", end_value=f"$H{top}*1.6",
                           end_color="FF63BE7B"))
...
def save(rows, cands, out=OUT):
    ...
    lines = [
        ...
        ("  цены брендов", "трёхцветная шкала образца, построчно, но пороги привязаны к "
                           "НАШЕЙ цене: ровно наша цена — жёлтый, 0,6 от неё и ниже — "
                           "красный, 1,6 и выше — зелёный. То есть красный всегда значит "
                           "«конкурент дешевле нас», а насыщенность показывает насколько. "
                           "У заказчика шкала растягивалась по ценам конкурентов и от нашей "
                           "не зависела: при одном аналоге он был и min и max, то есть "
                           "зелёным всегда (225 строк из 663). Коэффициенты 0,6 и 1,6 сняты "
                           "с данных: по 2 061 паре отношение «их цена / наша» имеет 5-й "
                           "квантиль 0,55, медиану 0,97 и 90-й 1,58."),

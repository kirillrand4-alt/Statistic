...
def test_builds_detailed_centro_snapshot(tmp_path, monkeypatch):
    summary = _write(
        ...
        "status_egrul;novost;novost_ssylka;telefony_iz_bazy;"
        "telefony_predpriyatiya;nomera_bez_vladelca;pochta;"
        "istochnik_dopolneniya;proverok_nadzora;ssylki_na_istochniki\n"
        "1234567890;Тестовый завод;центробежная;воздух;К-250;есть;ACTIVE;"
        "Модернизация;https://news.example/item;+7 999 000-00-01;;;"
        "office@example.ru;директор/ЕГРЮЛ;2;https://monitor-pb.example/check\n",
    )
    details = _write(
        tmp_path / "POLNYY.csv",
    ...
    assert facts[0]["model"] == "К-250"
    assert facts[0]["source_url"] == "https://source.example/fact"
    ...
    assert info["source_kind"] == "csv-import-v2"
